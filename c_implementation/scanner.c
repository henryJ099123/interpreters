...
// private scanner? so in the source file?
typedef struct {
...
static Token makeToken(TokenType type) {
	Token token;
	token.type = type;
	// sets fields based on the scanner's current info
	token.start = scanner.start;
	// uses `start` and `current` pointers to get the lexeme
...
static TokenType checkKeyword(int start, int length, const char* rest, TokenType type) {
	//if length is the same and memory contents the same
...
// very fast method of verification
...
static Token identifier() {
	// keep reading letters or numbers
...
static Token number() {
	...
	if(peek() == '.' && isDigit(peekNext())) {
		// consume "."
		advance();
		while(isDigit(peek())) advance(); // eat rest of number
	} 

	return makeToken(TOKEN_NUMBER);
} 

// includes the " and the " in the lexeme
static Token string() {
	while(peek() != '"' && !isAtEnd()) {
		if(peek() == '\n') scanner.line++;
		advance();
